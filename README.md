...
If already cloned, `cd` into the directory and run:
...
Assuming the cloned repo is in `./zsh-setup`, run:
...
This should trigger the theme configuration menu. Run `p10k configure` anytime to configure it again.
...
And enter `/usr/bin/zsh` as the path to the new shell. Or configure your desired terminal to use `zsh`.
...
This will uninstall `zsh` and its plugins and delete the theme from `$XDG_DATA_DIR`. It intentionally **does not** uninstall `ttf-hack-nerd`, as it may be being used by the terminal. To uninstall it manually, first make sure all terminals/configs are using a different font-family, then run:
...
For an up-to-date list, refer to [vars.sh](src/vars.sh). As of `v1.3`, the list of packages this tool un/installs:
